...
cordDF.ordered.Top10 <- data.frame(head(corDF.ordered, 10), row.names = NULL)
cordDF.ordered.Top10 <- cordDF.ordered.Top10[,1:4]
...
### __Top 10 Multicollinear Terms, Correlation Matrix:__
...
```{r VIF, echo=F, include=T, warning=F}

vifFrame <- vif(df.numeric)
vifFrame <- vifFrame[order(-vifFrame$VIF),]
vifFrame.Top7 <- data.frame(head(vifFrame, 7), row.names = NULL)
colnames(vifFrame.Top7) <- c("Variable", "Variance Inflation Factor")
vifFrame.Top7$`Variance Inflation Factor` <- round(vifFrame.Top7$`Variance Inflation Factor`, digits = 2)

```

### __Top 10 Multicollinear Terms, Variance Inflation Factor:__
```{r VIF Table, echo=F, include=T, warning=F}

kable(vifFrame.Top7, format="latex", booktabs = T)  %>%
  kable_styling(latex_options="striped", position = "center")
...
## **Quadratic Discriminant Analysis: External Cross-Valdiation**
...
lr.Accuracy.confusion.internal <- lr.AccuracyP.confusion.internal[1,]
lr.Sensitivity.confusion.internal <- lr.SpecSense.confusion.internal[1,]
lr.Specificity.confusion.internal <- lr.SpecSense.confusion.internal[2,]
lr.Precision.confusion.internal <- lr.SpecSense.confusion.internal[5,]
...
The stepwise selection model was tuned using k-fold internal cross-validation.  The `playoffs` coefficient was added back into the model as the project requirements ask for interpretation of that particular coefficient.  Additionally the "Hosmer and Lemeshow goodness of fit (GOF) test" was run on the fitted model to assess if the null hypothesis of: the model does not provide information to predict the outcome variable.  Based on the evidence the model that was fit for this project can reject the null, where the model can provide information to predict the outcome variable (x = 17.361, df = 8, p-value = 0.02656).  The stepwise selection model generated is listed below where Logit coefficients for each feature can also be found in the "Fitted Logistic Regression Model" portion of the appendix:
...
Several notable features that this model generated are several of the `action_types`. The odds ratios of Kobe making a shot are revealed once the coefficients are exponentiated. In the table below we can see that the odds ratios are all very large. This suggests that when Kobe performs one of the listed levels of `action_type`, his chances of making the shot are very high. The frequency of the `action_type` can also be ascertained by looking at the standard errors from the model output. Small standard errors for a coefficient suggest that there are a large number of observations with that attribute with a similar outcomes, and a large standard error would suggest that there are fewer observations with a similar outcome. When moving from the base `action_type` of "Alley Oop Dunk Shot" to "Driving Bank Shot" for every one unit increase in "Driving Bank Shot" the logit value for `shot_made_flag` increases by approximately 10.5 units; in terms of odds ratios, compared to using the "Alley Oop Dunk Shot," Kobe's chances of making the shot when using the "Driving Bank Shot" is at a factor of approximately 37080.  The "Driving Bank Shot" also has a standard error of approximately 535, this value is higher than the other action types suggesting that Kobe uses the "Driving Bank Shot" infrequently but rarely misses this shot type when he chooses to use it.  When a frequency table of the `action_type` for the whole dataset is created, the total number of times the "Driving Bank Shot" occurs is 1 - compared to 5 observations for "Jump Shot."  The pattern of high odds ratios and high standard errors exist for the top 5 shots; this suggests that Kobe rarely uses these `action_types` but almost always makes the shot if that particular `action_type` is used.
...
logistic_prediction =  df.preds[,c("recId","prob")] #df.preds %>% select(recId, prob)

#write.csv(logistic_prediction,"logistic_prediction.csv")
```

```{r top shots, echo=F, include=T, warning=F }
suppressWarnings(kable(head(topshots, n = 6),
      format="latex", booktabs = T)) %>%
  kable_styling(latex_options="striped", position = "center")
```

# **Shot Made Odds and Probabilities Analysis Using Logistic Regression**
...
There is sufficient evidence to suggest that shot_distance from the hoop is not significant (p-value < 0.053303) with respect to all other covariates. Although the p-value for shot_distance is barely outside alpha = 0.05, it still provides practical information on whether or not Kobe makes his shots.
...
```{r Probabilities, echo=F, include=T, warning=F, fig.height = 8, fig.width = 10, fig.align="center"}

dfTrain1 = dfTrain %>% mutate(distance_odds = exp(0.0053920735)*shot_distance + exp(0.00514126)) %>% mutate(distance_prob = distance_odds/(1+distance_odds))

par(mfrow=c(2,1))
plot(dfTrain1$distance_prob,dfTrain1$shot_distance, xlab="Shot Probability", ylab="Shot Distance", main="Shot Probability over Distance")
plot(dfTrain1$shot_distance,dfTrain1$distance_prob, xlab="Shot Distance", ylab="Shot Probability", main="Shot Distance over Probability")
par(mfrow=c(1,1))
```
...
```{r VIF for Appendix, echo=F, include=T, warning=F}

vifFrame.Appendix <- data.frame(vifFrame, row.names = NULL)
colnames(vifFrame.Appendix) <- c("Variable", "Variance Inflation Factor")
vifFrame.Appendix$`Variance Inflation Factor` <- round(vifFrame.Appendix$`Variance Inflation Factor`, digits = 2)

kable(vifFrame.Appendix, format="latex", booktabs = T)  %>%
  kable_styling(latex_options="striped", position = "center")
...
## Shot Location Imputing, toInt Conversion
```r
df[which(df$loc_y > 300),"shot_type"] <- "3PT Field Goal"
df.preds[which(df.preds$loc_y > 300),"shot_type"] <- "3PT Field Goal"
...
## Variable Eliminations, First-Pass
```r
...
## Variable Eliminations, Second Pass
```r
...
corDF.ordered <- data.frame(corDF[order(-corDF$cor),])
cordDF.ordered.Top10 <- data.frame(head(corDF.ordered, 10), row.names = F)
...
## Quadratic Discriminant Analysis: Internal CV and Model Development
```r
...
## Quadratic Discriminant Analysis: External CV
...
```
##  Quadratic Discriminant Analysis Confusion Matrix Table
```r
...
```
## Logistic Regression: Internal CV
...
## Logistic Regression: Internal vs. External CV
...
lr.Specificity.confusion.external, digits=5),round(
